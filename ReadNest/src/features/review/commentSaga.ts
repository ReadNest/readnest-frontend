...
function* reportComment(action: PayloadAction<CreateCommentReportRequest>) {
  try {
    ...
    if (response.success) {
      ...
      toast.success(
        "Bạn đã báo cáo bình luận thành công! Admin sẽ xem xét và xử lý trong thời gian sớm nhất."
      );
    } else {
...
function* fetchTop3RecentCommentsByUser(action: PayloadAction<string>) {
  try {
    ...
    const response: { data: GetCommentResponse[]; success: boolean } =
      yield call(() =>
        client.api.v1.Comment.top_3_recent_comments
          ._userName(action.payload)
          .$get()
      );
    if (response.success) {
...
function* fetchTop3MostLikedComments() {
  try {
    ...
    const response: { data: GetCommentResponse[]; success: boolean } =
      yield call(() => client.api.v1.Comment.top_3_most_liked_comments.$get());
    if (response.success) {
...
export default function* commentSaga() {
  ...
  yield takeLatest(
    fetchTop3RecentCommentsRequested.type,
    fetchTop3RecentCommentsByUser
  );
  yield takeLatest(
    fetchTop3MostLikedCommentsRequested.type,
    fetchTop3MostLikedComments
  );
}
